...
public class RecipeDataAccess implements IRecipeDataAccess {
    ...
    public boolean addRecipe(Recipe recipe) {
        ...
        try {
            ...
            String sql = "INSERT INTO recipe (creator_id, name, image, description, cookingtimeinsec, " + "difficultylevel, rating, status, create_datetime, update_datetime) " + "VALUES (:creator_id, :name, :image, :description, :cookingtimeinsec, " + ":difficultylevel, :rating, :status, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) RETURNING id";
    ...
    private Recipe mapToRecipe(Map<String, Object> row) {
        ...
        recipe.setCreatorId(row.get("creator_id") != null ? ((Number) row.get("creator_id")).longValue() : null);
        ...
        recipe.setCookingTimeInSec(row.get("cookingtimeinsec") != null ? ((Number) row.get("cookingtimeinsec")).intValue() : null);
        recipe.setDifficultyLevel(row.get("difficultylevel") != null ? ((Number) row.get("difficultylevel")).intValue() : null);
        ...
        recipe.setCreateDatetime(row.get("create_datetime") != null ? (Timestamp) row.get("create_datetime") : null);
        recipe.setUpdateDatetime(row.get("update_datetime") != null ? (Timestamp) row.get("update_datetime") : null);
    ...
    private void validateRecipe(Recipe recipe) {
        ...
        if (recipe.getCreatorId() == null || recipe.getName() == null || recipe.getCookingTimeInSec() == null || recipe.getDifficultyLevel() == null || recipe.getRating() == null || recipe.getStatus() == null) {
    ...
    private void validateIngredient(Ingredient ingredient) {
        ...
        if (ingredient.getName() == null || ingredient.getQuantity() == null || ingredient.getUom() == null) {
    ...
    private void insertIngredients(Recipe recipe) {
        ...
        String ingredientSql = "INSERT INTO recipe_ingredients (recipe_id, name, quantity, uom) " + "VALUES (:recipe_id, :name, :quantity, :uom)";
    ...
    private void deleteIngredients(Long recipeId) {
        ...
        try {
            ...
            int deletedRows = postgresDataAccess.upsertStatement(deleteIngredientsSql, deleteIngredientsParams);
    ...
    private void insertCookingSteps(Recipe recipe) {
        ...
        String stepSql = "INSERT INTO recipe_cooking_step (recipe_id, description, image) " + "VALUES (:recipe_id, :description, :image)";
    ...
    private List<Ingredient> getIngredientsForRecipe(Long recipeId) {
        ...
        for (Map<String, Object> row : result) {
            ...
            ingredient.setRecipeId(row.get("recipe_id") != null ? ((Number) row.get("recipe_id")).longValue() : null);
            ingredient.setName(row.get("name") != null ? (String) row.get("name") : null);
            ingredient.setQuantity(row.get("quantity") != null ? ((Number) row.get("quantity")).doubleValue() : null);
    ...
    private List<CookingStep> getCookingStepsForRecipe(Long recipeId) {
        ...
        for (Map<String, Object> row : result) {
            ...
            step.setRecipeId(row.get("recipe_id") != null ? ((Number) row.get("recipe_id")).longValue() : null);
